...
__global__ void A_star(int* off,int* edge,int* W,int* Hx,int* Cx,int* P,int* PQ,int* PQS,int* L,int* nextFlag,int* endF,int N,int E,int K,int dest){
    ...
    if(id < K && PQS[id]>0){
        ...
        while(2*pqIndex+1 < PQS[id]){
            if(2*pqIndex+2 >= PQS[id]){
                ...
                if( Cx[PQ[front+pqIndex]] > Cx[PQ[front+2*pqIndex+1]] && Cx[PQ[front+2*pqIndex+1]] <= Cx[PQ[front+2*pqIndex+1]] ){
                    int swap = PQ[front + 2*pqIndex+1];
                    PQ[front + 2*pqIndex+1] = PQ[front +pqIndex];
                    PQ[front + pqIndex] = swap;
                    pqIndex = 2*pqIndex+1;
...
//for K in parallel
__global__ void insertPQ(int* PQ,int* PQS,int* Cx,int* nextV,int* nVsize,int K,int N){
    int id = blockIdx.x*blockDim.x+threadIdx.x;
    if(id < K){
        // printf("id: %d\n",id);
        int front = id*( (N+K-1)/K );
...
int main(){
    ...
    *a0 = 0;            // to set sizes 0
    ...
    cudaMemcpy(D_cx,H_cx,sizeof(int)*N,cudaMemcpyHostToDevice);
    ...
    while(*H_flagEnd==0 && flagQempty==1){
        ...
        //launch the A* kernel
        A_star<<<numThreads,numBlocks>>>(D_offset,D_edges,D_weights,D_hx,D_cx,D_parent,D_Kqueue,D_Kq_size,D_lock,D_nextFlag,D_flagEnd,N,edge_size,K,endNode);
        ...
        insertPQ<<<numThreads,numBlocks>>>(D_Kqueue,D_Kq_size,D_cx,D_nextV,D_nV_size,K,N);
