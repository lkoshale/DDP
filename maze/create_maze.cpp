...
int main()
{
    ...
    for(int cn=0;cn<counter;cn++){
        ...
        for(int i=0;i<c_size;i++){
            ...
            if(c_i > 0  ){
                node->remove_edge(Graph[(c_i-1)*M+c_j]);
                Graph[(c_i-1)*M+c_j]->remove_edge(node);
                
                str+="0 "+to_string(n)+" "+to_string((c_i-1)*M+c_j)+" 1\n";
                str+="0 "+to_string((c_i-1)*M+c_j)+" "+to_string(n)+" 1\n";
            ...
            if(c_j < M - 1  ){
                node->remove_edge(Graph[c_i*M+(c_j+1)]);
                Graph[c_i*M+(c_j+1)]->remove_edge(node);

                str+="0 "+to_string(n)+" "+to_string(c_i*M+(c_j+1))+" 1\n";
                str+="0 "+to_string(c_i*M+(c_j+1))+" "+to_string(n)+" 1\n";
