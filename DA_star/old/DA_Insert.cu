...
//for Edges N in parallel
__global__ void propogateInsert(int* diff_off,int* diff_edge,int* diff_weight,int N,int dE,
    int* Hx,int* lock,int* openList){
...
int main(){
    ...
    *H_a0 = 0;
    ...
    FILE* fdiff = fopen("diff","r");

    int dE;
    fscanf(fdiff,"%d\n",&dE);

    for(int i=0;i<dE;i++){
        fscanf(fdiff,"%d",&H_diff_edges[i]);
    }

    for(int i=0;i<N;i++){
        fscanf(fdiff,"%d",&H_diff_offset[i]);
    }

    for(int i=0;i<dE;i++){
        fscanf(fdiff,"%u",&H_diff_weight[i]);
    }

    //DElete
    /*
    int u,v;
    while(fscanf(fdiff,"%d %d\n",&u,&v)!=EOF){
        //delete from graph
    ...
    */
    ...
    //first remove deleted edges
    //propogate insert (add dest too if not deleted)
    //remake priorrity queue
    //do A* 
    //prop delete
