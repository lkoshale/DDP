        ...
        while(start < end){ 
            ...
            while(leaveLoop==false){
                ...
                if(atomicCAS(&lock[child],0,1)==0){
                    //critical section
                    float exp_weight = 1 / ( ALPHA*(res_energy[child]/E_INIT) + BETA*(Nr[child]/Nt[child]) + DELTA*0) ;
...
//for K in parallel
__global__ void checkMIN(int* PQ_size,int* flagEnd,float* Cx,int dest,int N,int K){
    int id = blockIdx.x*blockDim.x+threadIdx.x;
